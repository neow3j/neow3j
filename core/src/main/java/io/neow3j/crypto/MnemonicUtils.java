 ...
 *
 * @see
 * <a href="https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki">Mnemonic code for generating deterministic keys</a>
 */
public class MnemonicUtils {
    ...
    /**
     * The mnemonic must encode entropy in a multiple of 32 bits. With more entropy security is improved but the
     * sentence length increases. We refer to the initial entropy length as ENT. The allowed size of ENT is 128-256
     * bits.
     * <p>
     * <b>Mnemonic generation algorithm:</b> Given a randomly generated initial entropy of size ENT, first a checksum
     * is generated by taking the first {@code ENT / 32} bits of its SHA256 hash. This checksum is appended to the
     * end of the initial entropy. Next, these concatenated bits are split into groups of 11 bits, each encoding a
     * number from 0-2047, serving as an index into a wordlist. Finally, we convert these numbers into words and use
     * the joined words as a mnemonic sentence.
     *
     ...
     * @return the byte array representation of the entropy.
     */
     ...
     * used as the password and the string "mnemonic" + passphrase (again in UTF-8 NFKD) used as the salt. The
     * iteration count is set to 2048 and HMAC-SHA512 is used as the pseudo-random function. The length of the
     * derived key is 512 bits (= 64 bytes).
     *
     * @param mnemonic   the input mnemonic which should be 128-160 bits in length containing only valid words.
     * @param passphrase the passphrase which will be used as part of salt for PBKDF2 function.
     * @return the byte array representation of the generated seed.
     */
    public static byte[] generateSeed(String mnemonic, String passphrase) {
        ...
        String salt = format("mnemonic%s", passphrase);
    ...
    private static String mnemonicToBits(String mnemonic) {
        ...
        for (String word : words) {
            ...
            if (index == -1) {
                throw new IllegalArgumentException(format("Mnemonic word '%s' should be in the word list", word));
            }
