...
if (echo "${TRAVIS_TAG}" | grep -E -q ${SNAPSHOT_TAG_PATTERN}) || (echo "${TRAVIS_TAG}" | grep -E -q ${RELEASE_TAG_PATTERN}); then
    ...
    if [ "${TRAVIS_PULL_REQUEST}" == 'false' ]; then
        ...
        ./gradlew releaseAndClose -PnexusUsername=${SONATYPE_USERNAME} -PnexusPassword=${SONATYPE_PASSWORD} -Psigning.keyId=${GPG_KEY_ID} -Psigning.password=${GPG_KEY_PASSPHRASE} -Psigning.secretKeyRingFile=${GPG_FILE} ${PARAMS}
    fi
fi