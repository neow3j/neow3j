...
/**
 * Represents a smart contract on the Neo blockchain and provides methods to
 * invoke and deploy it.
 */
public class SmartContract {
    ...
    /**
     * Constructs a {@code SmartContract} representing the smart contract with
     * the given script hash. Uses the given {@link Neow3j} instance for all
     * invocations.
     *
    ...
    public SmartContract(ScriptHash scriptHash, Neow3j neow) {
        if (scriptHash == null) {
            throw new IllegalArgumentException("The contract script hash " +
                    "must not be null.");
        }
        if (neow == null) {
            throw new IllegalArgumentException("The Neow3j object must not " +
                    "be null.");
        }
    ...
    /**
     * Initializes a {@link TransactionBuilder} for an invocation of this
     * contract with the provided function and parameters. The order of the
     * parameters is relevant.
     *
     * @param function the function to invoke.
     * @param contractParameters the parameters to pass with the invocation.
     * @return a {@link TransactionBuilder} allowing to set further details of
     *         the invocation.
     */
     ...
     * expecting a String as return type.
     *
     ...
     *                     Neo node.
     * @throws UnexpectedReturnTypeException if the returned type could not be
    ...
    /**
     * Sends an {@code invokefunction} RPC call to the given contract function
     * expecting an Integer as return type.
     *
     * @param function the function to call.
     * @param params the contract parameters to include in the call.
     ...
     */
    public BigInteger callFuncReturningInt(String function,
            ContractParameter... params)
            throws IOException, UnexpectedReturnTypeException {

        StackItem item;
        if (params.length == 0) {
            item = callInvokeFunction(function)
                    .getInvocationResult().getStack().get(0);
        } else {
    ...
    /**
     * Sends an {@code invokefunction} RPC call to the given contract function
     * expecting a Boolean as return type.
     *
     * @param function the function to call.
     * @param params the contract parameters to include in the call.
     ...
     *                     Neo node.
     * @throws UnexpectedReturnTypeException if the returned type could not be
     *                                       interpreted as an boolean.
     */
    public boolean callFuncReturningBool(String function,
            ContractParameter... params)
            throws IOException, UnexpectedReturnTypeException {
     ...
     * @throws IOException if something goes wrong when communicating with the
     *                     Neo node.
     */
    public NeoInvokeFunction callInvokeFunction(String function,
    ...
    public NeoInvokeFunction callInvokeFunction(String function,
        ...
        return neow.invokeFunction(scriptHash.toString(), function,
                params, signers).send();
    }
    ...
    public ContractManifest getManifest() throws IOException {
        ContractState contractState =
                neow.getContractState(scriptHash.toString()).send()
                        .getContractState();
     ...
     * <p>
     * A contract's hash doesn't change after deployment. Even if the contract's
     * script is updated the hash stays the same. It depends on the initial NEF
     * checksum, contract name, and the account that sent the deployment
     * transaction.
     *
     ...
     */
    public static ScriptHash getContractHash(ScriptHash sender,
            long nefCheckSum, String contractName) {
