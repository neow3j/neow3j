...
/**
 * Represents the NeoToken native contract and provides methods to invoke
 * its functions.
 */
public class NeoToken extends Nep17Token {
    ...
    public final static BigInteger TOTAL_SUPPLY =
    ...
    public static final String GET_NEXT_BLOCK_VALIDATORS =
    ...
    /**
     * Constructs a new {@code NeoToken} that uses the given {@link Neow3j}
    ...
    /**
     * Returns the name of the NeoToken contract. Doesn't require a call to the
     * Neo node.
     *
     * @return the name.
     */
    ...
    /**
     * Returns the symbol of the NeoToken contract. Doesn't require a call to
     * the Neo node.
     *
     * @return the symbol.
     */
    ...
    /**
     * Returns the total supply of the NeoToken contract. Doesn't require a
     * call to the Neo node.
     *
    ...
    /**
     * Returns the number of decimals of the NEO token. Doesn't require a call
     * to the Neo node.
     *
    ...
    /**
     * Gets the amount of unclaimed GAS at the given height for the given
     * account.
     *
     ...
     *                     Neo node.
     */
    public BigInteger unclaimedGas(Account account, long blockHeight)
            throws IOException {
     ...
     *                     Neo node.
     */
    public BigInteger unclaimedGas(ScriptHash scriptHash, long blockHeight)
            throws IOException {
        ContractParameter accParam = hash160(scriptHash);
        ContractParameter heightParam =
                integer(BigInteger.valueOf(blockHeight));
        return callFuncReturningInt(UNCLAIMED_GAS, accParam, heightParam);
    }

    /**
     * Creates a {@code TransactionBuilder} for registering a committee
     * candidate with the given public key.
     * <p>
     * Note, that the transaction has to be signed with the account
    ...
    public TransactionBuilder registerCandidate(ECPublicKey candidateKey) {
        ...
        return invokeFunction(REGISTER_CANDIDATE,
                publicKey(candidateKey.getEncoded(true)));
    }

    /**
     * Creates a transaction script for registering a validator candidate and
     ...
     * @throws IOException if there was a problem fetching information from the
     *                     Neo node.
     * @throws UnexpectedReturnTypeException if the return type is not an array
     *                                       or the returned array's elements
     *                                       are not public keys.
     */
    ...
    /**
     * Gets the public keys of currently registered validator candidates and
     * their NEO balances.
     *
     ...
     *                     Neo node.
     * @throws UnexpectedReturnTypeException if the return type is not an array
     *                                       or the array elements are not
     *                                       public keys and node counts.
     */
    public Map<ECPublicKey, Integer> getCandidates() throws IOException {
        ...
        if (!arrayItem.getType().equals(StackItemType.ARRAY)) {
            throw new UnexpectedReturnTypeException(arrayItem.getType(),
                    StackItemType.ARRAY);
        }
        ...
        for (StackItem valItem : arrayItem.asArray().getValue()) {
            if (!valItem.getType().equals(StackItemType.STRUCT)) {
                throw new UnexpectedReturnTypeException(valItem.getType(),
                        StackItemType.STRUCT);
            }
            ECPublicKey key = extractPublicKey(valItem
                    .asStruct().getValue().get(0));
            StackItem nrItem = valItem.asStruct().getValue().get(1);
            if (!nrItem.getType().equals(StackItemType.INTEGER)) {
                throw new UnexpectedReturnTypeException(nrItem.getType(),
    ...
    private List<ECPublicKey> callFunctionReturningListOfPublicKeys
            (String function) throws IOException {

        StackItem arrayItem = callInvokeFunction(function)
                .getInvocationResult().getStack().get(0);
        if (!arrayItem.getType().equals(StackItemType.ARRAY)) {
            throw new UnexpectedReturnTypeException(arrayItem.getType(),
                    StackItemType.ARRAY);
    ...
    private ECPublicKey extractPublicKey(StackItem keyItem) {
        ...
        try {
            ...
            throw new UnexpectedReturnTypeException("Byte array return type" +
                    " did not contain public key in expected format.", e);
        }
    }
     ...
     * <p>
     * This contract invocation can only be successful if it is signed by the
     * network committee.
     *
