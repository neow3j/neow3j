...
if [ "${SNAPSHOT}" == "${TRAVIS_TAG}" ] || [ "${RELEASE}" == "${TRAVIS_TAG}" ]; then
    ...
    if [ "${TRAVIS_PULL_REQUEST}" == "false" ]; then
        ...
        # publish all modules
        ./gradlew publish -Psonatype -PnexusUsername=${SONATYPE_USERNAME} -PnexusPassword=${SONATYPE_PASSWORD} -Psigning.keyId=${GPG_KEY_ID} -Psigning.password=${GPG_KEY_PASSPHRASE} -Psigning.secretKeyRingFile=${GPG_FILE} ${PARAMS}
        ...
        if [ $? -eq 0 ] && [ "${RELEASE}" == "${TRAVIS_TAG}" ]; then
            ...
            ./gradlew closeAndReleaseRepository -Psonatype -PnexusUsername=${SONATYPE_USERNAME} -PnexusPassword=${SONATYPE_PASSWORD} -Psigning.keyId=${GPG_KEY_ID} -Psigning.password=${GPG_KEY_PASSPHRASE} -Psigning.secretKeyRingFile=${GPG_FILE} ${PARAMS}
        fi
    fi
fi